...
def _fake_llm_port(manifest: Manifest) -> object:
    """Create a fake LLM port that records calls and returns a manifest."""
...
def _get_default_manifest() -> Manifest:
    """Fetch the default manifest from a no-LLM stream response."""
    with patch(
...
def _make_refined(base: Manifest) -> Manifest:
    """Tweak importance scores so the manifest differs from default."""
    return Manifest(
        items=[
            ManifestItem(
                id=i.id,
                importance=min(i.importance + 0.1, 1.0),
                molecule=i.molecule,
                data=i.data,
            )
            for i in base.items
...
class TestAgentCaching:
    ...
    def test_given_cached_manifest_when_same_referrer_then_llm_skipped(
        self,
    ) -> None:
        """Cache hit returns cached manifest without calling LLM."""
        default = _get_default_manifest()
